...
for imgLeft, imgRight in zip(imagesLeft, imagesRight):
    ...
    if imgL is None or imgR is None:
        print(f"Error: One of the images did not load correctly (Left: {imgLeft}, Right: {imgRight})")
        continue
    ...
    # Debugging: Print whether corners were found
    print(f"Chessboard detected in left image: {retL}, right image: {retR}")
    ...
    if retL and retR:
        ...
        print(f"Warning: Chessboard not found in both images for this pair (Left: {imgLeft}, Right: {imgRight})")

cv.destroyAllWindows()

# Check if we have enough points to proceed with calibration
if not objpoints or not imgpointsL or not imgpointsR:
    print("Error: Not enough points were found for calibration. Check your images and chessboard detection.")
    exit()
...
flags = 0
flags |= cv.CALIB_FIX_INTRINSIC
# Fix intrinsic camera matrices so only rotation, translation, essential, and fundamental matrices are calculated.

criteria_stereo = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 30, 0.001)

# Perform stereo calibration to find the transformation between the two cameras
...
print("Stereo calibration and rectification parameters saved successfully.")
